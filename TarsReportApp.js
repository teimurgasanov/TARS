  ...
  "build_0.5.3/upload-duplicate-guard.js"(exports2, module2) {
    ...
    function createReceiptProcessingStatusManager(message, read, persistence, modify, logger, allowPublish = true) {
      ...
      const mark = (handle, stage, startedAt, outcome) => {
        if (handle) logReceiptStage(logger, handle.context, stage, startedAt, outcome);
      };
      const markAll = (stage, outcome) => {
    ...
    async function archiveReceiptYandex(file, content, receiptCheck, exact, read, persistence, http, config, logger) {
      ...
      const archived = {
        externalArchiveKey: objectKey,
        externalArchiveBucket: config.archiveBucket,
        externalArchiveStatus: "stored",
        externalArchivedAt: now,
      ...
      if (logger) logger.info(`Receipt ${String(exact || "").slice(0, 16)} stored in Yandex Object Storage`);
      return archived;
    }
    const RECEIPT_REVIEW_ROOM = "cheki-kontrol";
    const APPROVE_REJECTED_RECEIPT_ACTION = "approve-rejected-receipt";
    const RECEIPT_CASH_RETENTION_MS = 24 * 60 * 60 * 1e3;
    const RECEIPT_ARCHIVE_RETENTION_MS = 60 * 24 * 60 * 60 * 1e3;
    ...
    function reportForwardSourceMessageId(message) {
      const text = String(message && message.text || "");
      const textMatch = text.match(/[?&]msg=([^\s)&]+)/i);
      ...
      for (const attachment of message && Array.isArray(message.attachments) ? message.attachments : []) {
        const candidates = [
          attachment && attachment.messageLink,
          attachment && attachment.message_link,
          attachment && attachment.title && attachment.title.link
        ];
        for (const candidate of candidates) {
          const match = String(candidate || "").match(/[?&]msg=([^\s)&]+)/i);
    ...
    function reportForwardIdentity(message) {
      const sourceMessageId = reportForwardSourceMessageId(message);
    ...
    function isTarsReportForwardMessage(message) {
      ...
      if (reportForwardSourceMessageId(message)) return true;
    ...
    async function findArchiveMessageByUploadId(roomId, uploadId, read, sourceMessageId = "") {
      ...
      for (let page = 0; page < 10; page += 1) {
        ...
        const found = (messages || []).find((message) => {
          const messageRoomId = String(message && (message.roomId || message.room && message.room.id) || "");
          const sameSourceMessage = sourceMessageId && reportForwardSourceMessageId(message) === String(sourceMessageId);
          return messageRoomId === String(roomId) && (sameSourceMessage || reportForwardMatchesUpload(message, uploadId));
        });
    ...
    async function archiveTextMessage(oldMessage, room, read, modify, config, logger) {
      // Personal chat text is intentionally not copied to Rocket.Chat archive
    ...
    async function forwardReportPhotoMessage(sourceMessage, sourceFile, sourceUpload, room, appUser, modify, logger, details) {
      ...
      const permalink = await rocketChatForwardPermalink(sourceMessage, sourceFile, sourceUpload, details && details.read);
      if (!permalink) return "";
      // Rocket.Chat's own Forward action posts a hidden permalink to the
      // original message. The server expands it into a quote attachment and
      // keeps the original photo, author and source message together. Reuse
      // that native path instead of attaching an upload from another room.
      const text = `[ ](${permalink})`;
      const builder = modify.getCreator().startMessage().setSender(appUser).setRoom(room).setText(text).setParseUrls(true);
      const messageId = await modify.getCreator().finish(builder);
      if (logger) logger.info(`Report photo forwarded with Rocket.Chat native permalink: message=${messageId || "unknown"} sourceMessage=${sourceMessage && sourceMessage.id || "unknown"} sourceUpload=${uploadId}`);
      return String(messageId || "");
    }
    async function cleanupDuplicateReportForwards(roomId, uploadId, read, modify, appUser, logger, sourceMessageId = "") {
      ...
      for (let attempt = 0; attempt < 4; attempt += 1) {
        ...
        for (let page = 0; page < 3; page += 1) {
          ...
          for (const candidate of messages || []) {
            ...
            const sameSourceMessage = sourceMessageId && reportForwardSourceMessageId(candidate) === String(sourceMessageId);
            if (sameSourceMessage || reportForwardMatchesUpload(candidate, uploadId)) matches.push(candidate);
          }
          if (!messages || messages.length < 100) break;
        }
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          try {
            ...
            if (diagnostic) diagnostic.dedicated_parser = "parse_error";
            throw parseError;
          }
          if (diagnostic) diagnostic.dedicated_parser = openAiDedicatedParserState(text, parsed);
          ...
          if (diagnostic && diagnostic.dedicated_transport !== "2xx") diagnostic.dedicated_transport = /timeout|timed\s*out|etimedout/i.test(String(error && error.message || error)) ? "timeout" : "other_error";
    ...
    async function detectPersonalMailingProof(message, read, http, config, logger) {
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        if (attempt > 0) await new Promise((resolve) => setTimeout(resolve, 1e3));
        let readableFileSeen = false;
        for (const file of imageFiles) {
          const uploadId = String(file && (file._id || file.id) || "");
          if (!uploadId) continue;
          try {
            const content = await read.getUploadReader().getBufferById(uploadId);
            readableFileSeen = true;
            if (await personalImageKindForPreUpload(file, content, http, config, logger) === "mailing") {
              return { file, uploadId };
            }
          } catch (error) {
            if (logger) logger.warn(`AUTO_MAILING_PROOF_READ_FAILED attempt=${attempt + 1} upload=${uploadId}: ${error && error.message || error}`);
          }
        }
        // Retry only while Rocket.Chat has not made the uploaded bytes
        // readable yet. Re-running OCR/Vision eight times for a readable
        // non-mailing image can exhaust the post-message execution window and
        // prevent both photo confirmation and receipt accounting.
        if (readableFileSeen) return void 0;
      }
      return void 0;
    }
    async function fastForwardPersonalReportPhotos(message, read, persistence, modify, logger, http, config, explicitPhotoIntent = false, diagnostic) {
      ...
      let bestCandidate;
      // Rocket.Chat may publish the message event before a mobile upload can
      // be read through UploadReader. Give the file enough time to settle so
      // a valid work photo is not silently skipped on a slow connection.
      for (let attempt = 0; attempt < 8 && !bestCandidate; attempt += 1) {
        if (attempt > 0) await new Promise((resolve) => setTimeout(resolve, 1e3));
      ...
      if (!bestCandidate) {
        if (logger) logger.warn(`FAST_PHOTO_FORWARD_FILE_UNAVAILABLE uploads=${imageFiles.map((file) => String(file && (file._id || file.id) || "")).filter(Boolean).join(",") || "none"}`);
        return false;
      }
      const sourceFile = bestCandidate.sourceFile;
      const uploadId = bestCandidate.uploadId;
      capturePersonalImageSourceTelemetry(diagnostic, sourceFile, bestCandidate.content);
      ...
      try {
        ...
        if (!reportMessageId) throw new Error("Rocket.Chat did not create the native forwarded report photo message");
        entry.reportRoomId = room.id;
        ...
        await writeIndex(persistence, indexName, index);
        await cleanupDuplicateReportForwards(room.id, uploadId, read, modify, appUser, logger, message.id);
    ...
    async function publishDirectReportPhotos(message, read, persistence, modify, logger, entries = [], index, indexName, http, config) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          const content = await read.getUploadReader().getBufferById(uploadId);
          const confirmedDecision = await shouldForwardConfirmedWorkPhoto(sourceFile, content, http, config, logger, false);
          if (!confirmedDecision.forward) {
            entry.reportMessageId = "blocked_not_work_photo";
            entry.reportPublishedAt = Date.now();
            delete entry.reportPublishLockUntil;
            published = true;
            if (logger) logger.info(`DIRECT_PHOTO_FORWARD_BLOCKED_NOT_WORK_PHOTO upload=${uploadId} reason=${confirmedDecision.reason || "unknown"}`);
            continue;
          }
          const existingForward = await findArchiveMessageByUploadId(room.id, uploadId, read, message.id);
          if (existingForward) {
            entry.reportRoomId = room.id;
            entry.reportUploadId = uploadId;
    ...
    async function archiveUploadExists(entry, read) {
      if (entry && entry.externalArchiveStatus === "stored" && entry.externalArchiveKey) return true;
    ...
    async function archiveReceipt(file, content, receiptCheck, exact, read, persistence, modify, config, logger, http) {
      if (!config || !config.archiveEnabled) return null;
      if (!archiveConfigured(config)) throw new Error("Yandex Object Storage archive is not configured");
      if (!http) throw new Error("Yandex Object Storage HTTP client is unavailable");
      ...
      const existing = existingRecords.find((entry) => entry.externalArchiveStatus === "stored" && entry.externalArchiveKey);
      if (existing) return existing;
      const yandex = await archiveReceiptYandex(file, content, receiptCheck, exact, read, persistence, http, config, logger);
      const archived = {
        ...yandex,
        archiveId: String(exact || yandex.externalArchiveKey || "").slice(0, 16),
        archiveKey: `yandex:${yandex.externalArchiveKey}`,
        archiveBackend: "yandex-object-storage-v1",
        archiveStatus: "stored",
        archivedAt: yandex.externalArchivedAt || Date.now(),
        externalArchiveError: ""
      };
      await persistence.createWithAssociation(archived, archiveDayAssociation(archived.receiptDate));
      if (logger) logger.info(`Receipt ${archived.archiveId} stored only in Yandex Object Storage`);
      return archived;
    }
    ...
    function protectedRoomForSlug(slug) {
      ...
      if (!normalized || normalized === "general" || normalized === RECEIPT_REVIEW_ROOM || normalized.indexOf("arhiv") !== -1 || normalized.indexOf("архив") !== -1 || normalized.indexOf("kontrol") !== -1 || normalized.indexOf("контрол") !== -1) return void 0;
      if (PROTECTED_ROOMS[normalized]) return PROTECTED_ROOMS[normalized];
      if (normalized.indexOf("result") !== -1 || normalized.indexOf("результат") !== -1 || normalized.indexOf("otchet") !== -1 || normalized.indexOf("отчет") !== -1 || normalized.indexOf("отчёт") !== -1) return PROTECTED_ROOMS.otchet;
    ...
    function isArchiveRoom(room) {
      const slug = String(room && room.slugifiedName || "").toLowerCase();
      const name = String(room && (room.displayName || room.name || "") || "").toLowerCase();
      return Boolean(slug === RECEIPT_REVIEW_ROOM || slug === "receipt-archive" || slug.indexOf("arhiv") !== -1 || slug.indexOf("archive") !== -1 || slug.indexOf("kontrol") !== -1 || slug.indexOf("control") !== -1 || slug.indexOf("архив") !== -1 || slug.indexOf("контрол") !== -1 || name.indexOf("архив") !== -1 || name.indexOf("archive") !== -1 || name.indexOf("контрол") !== -1 || name.indexOf("control") !== -1);
    }
    async function isKnownArchiveRoom(room, read) {
      ...
      try {
        const reviewRoom = await read.getRoomReader().getByName(RECEIPT_REVIEW_ROOM);
        return Boolean(reviewRoom && reviewRoom.id && String(reviewRoom.id) === String(room.id));
      } catch (_2) {
        return false;
      }
    }
    ...
    function messageLooksLikePendingImageUpload(message) {
      const text = String(message && message.text || "").trim();
    ...
    async function resolvePersonalImageMessageV2(initialMessage, read, logger, maxAttempts = 16, delayMs = 750, expectMedia = true) {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
          ...
          if (logger) logger.warn(`MEDIA_V2_MESSAGE_READ_FAILED message=${messageId} attempt=${attempt + 1} error=${error && error.message || error}`);
        }
        try {
          const recent = await read.getRoomReader().getMessages(roomId, {
            limit: 20,
            sort: { createdAt: "desc" },
            showThreadMessages: true
          });
          ...
          // second message instead of attaching it to the preliminary event.
          // Only accept a later image from the same sender in a narrow window;
          ...
          if (logger) logger.warn(`MEDIA_V2_ROOM_READ_FAILED message=${messageId} attempt=${attempt + 1} error=${error && error.message || error}`);
        }
      }
    ...
    const PERSONAL_IMAGE_PIPELINE_TELEMETRY_KEYS = [
      "source_type",
      "extension_enum",
      "declared_mime_enum",
    ...
    function createPersonalImageClassificationDiagnostic(sourceType = "unknown") {
      return {
        ...
        extension_enum: "unknown",
        declared_mime_enum: "unknown",
        detected_mime_by_magic_bytes: "unknown",
        mime_match: "unknown",
        size_bucket: "<250kb",
        primary_cache: "miss",
        dedicated_cache: "miss",
        strict_cache: "miss",
        primary_transport: "other_error",
        primary_parser: "no_json",
        primary_normalized_result: "unknown",
        dedicated_transport: "other_error",
        dedicated_parser: "no_json",
        dedicated_normalized_result: "unknown",
        receipt_openai_transport: "other_error",
    ...
    function personalImageExtensionEnum(file) {
      const title = file && file.title;
      const titleValue = String(title && typeof title === "object" ? title.value : title || "");
      const name = String(file && (file.name || titleValue || file.url || file.path) || "").split("?")[0].toLowerCase();
      const match = name.match(/\.([a-z0-9]+)$/);
      const extension = String(match && match[1] || "");
      return ["jpg", "jpeg", "png", "webp", "heic", "heif"].indexOf(extension) !== -1 ? extension : "unknown";
    }
    function personalImageDeclaredMimeEnum(file) {
      const mime = String(file && (file.type || file.mimeType) || "").trim().toLowerCase().split(";")[0];
    ...
    function detectedPersonalImageMimeByMagicBytes(content) {
      if (!content || Number(content.length || 0) < 4) return "unknown";
      const byte = (index) => Number(content[index]);
      if (byte(0) === 255 && byte(1) === 216 && byte(2) === 255) return "jpeg";
      if (Number(content.length || 0) >= 8 && byte(0) === 137 && byte(1) === 80 && byte(2) === 78 && byte(3) === 71 && byte(4) === 13 && byte(5) === 10 && byte(6) === 26 && byte(7) === 10) return "png";
      const ascii = (start, length) => {
        let value = "";
        for (let index = start; index < start + length && index < Number(content.length || 0); index += 1) value += String.fromCharCode(byte(index));
        return value;
      };
      if (Number(content.length || 0) >= 12 && ascii(0, 4) === "RIFF" && ascii(8, 4) === "WEBP") return "webp";
      if (Number(content.length || 0) >= 12 && ascii(4, 4) === "ftyp") {
        const brands = [];
        for (let offset = 8; offset + 4 <= Math.min(Number(content.length || 0), 64); offset += 4) brands.push(ascii(offset, 4).toLowerCase());
        if (brands.some((brand) => /^(?:heic|heix|hevc|hevx)$/.test(brand))) return "heic";
        if (brands.some((brand) => /^(?:heif|mif1|msf1)$/.test(brand))) return "heif";
      }
      return "unknown";
    }
    function personalImageSizeBucket(content) {
      const size = Number(content && content.length || 0);
      if (size < 250 * 1024) return "<250kb";
      if (size < 1024 * 1024) return "250kb-1mb";
      if (size <= 5 * 1024 * 1024) return "1-5mb";
      return ">5mb";
    }
    function capturePersonalImageSourceTelemetry(diagnostic, file, content) {
      if (!diagnostic) return;
      const declared = personalImageDeclaredMimeEnum(file);
      const detected = detectedPersonalImageMimeByMagicBytes(content);
      diagnostic.extension_enum = personalImageExtensionEnum(file);
      diagnostic.declared_mime_enum = declared;
      diagnostic.detected_mime_by_magic_bytes = detected;
      diagnostic.mime_match = declared === "unknown" || detected === "unknown" ? "unknown" : declared === detected;
      diagnostic.size_bucket = personalImageSizeBucket(content);
    }
    function capturePersonalImageCacheTelemetry(diagnostic, field, hit) {
      if (!diagnostic || ["primary_cache", "dedicated_cache", "strict_cache"].indexOf(field) === -1) return;
      const seenField = `_${field}_observed`;
      if (diagnostic[seenField]) return;
      diagnostic[field] = hit ? "hit" : "miss";
      diagnostic[seenField] = true;
    }
    function personalImageTransportEnum(value) {
      return personalImageDiagnosticEnum(value, ["2xx", "429", "5xx", "timeout", "other_error"], "other_error");
    }
    function personalImageParserEnum(value) {
      return personalImageDiagnosticEnum(value, ["parsed", "no_json", "schema_mismatch", "parse_error"], "no_json");
    }
    function openAiReceiptParserState(text, parsed) {
    ...
    function openAiDedicatedParserState(text, parsed) {
      ...
      if (!source) return "no_json";
      if (!parsed) return source.indexOf("{") !== -1 ? "parse_error" : "no_json";
      if (typeof parsed !== "object" || Array.isArray(parsed) || typeof parsed.is_work_photo !== "boolean" || typeof parsed.is_document_or_screen !== "boolean" || typeof parsed.is_receipt_or_banking !== "boolean" || typeof parsed.has_visible_client !== "boolean" || typeof parsed.has_visible_service_area !== "boolean" || typeof parsed.kind !== "string") return "schema_mismatch";
      return "parsed";
    }
    function normalizedPrimaryProviderResult(parsed, candidate, parserState) {
      if (parserState !== "parsed") return "unknown";
      if (aiCandidateMarksMailing(candidate)) return "mailing";
      if (aiCandidateMarksReceipt(candidate)) return "receipt";
    ...
    function captureOpenAiReceiptTelemetry(diagnostic, role, transport, parserState, parsed, candidate) {
      if (!diagnostic) return;
      if (role === "primary") {
        diagnostic.primary_transport = personalImageTransportEnum(transport);
        diagnostic.primary_parser = personalImageParserEnum(parserState);
        diagnostic.primary_normalized_result = normalizedPrimaryProviderResult(parsed, candidate, parserState);
        return;
      }
      if (role === "receipt") {
        diagnostic.receipt_openai_transport = personalImageTransportEnum(transport);
        diagnostic.receipt_openai_parser = personalImageParserEnum(parserState);
        diagnostic.receipt_openai_result = parserState !== "parsed" ? "unknown" : aiCandidateMarksReceipt(candidate) ? "receipt" : "not_receipt";
      }
    }
    function personalImageStrictResult(result) {
      if (result && result.ok) return "accept";
      const reason = String(result && result.reason || "").toLowerCase().replace(/ё/g, "е");
      if (/не распознан|не удалось проверить|не совпал|нужна проверка|не подтвержден|ожидан|обработ/.test(reason)) return "review";
      return "reject";
    }
    ...
    function captureDedicatedWorkPhotoDiagnostic(diagnostic, parsed) {
      ...
      diagnostic.dedicated_normalized_result = diagnostic.dedicated_result;
      diagnostic._dedicated_schema_observed = true;
    }
    ...
    function mergePersonalImageReceiptTelemetry(target, source) {
      if (!target || !source) return;
      for (const key of ["receipt_openai_transport", "receipt_openai_parser", "receipt_openai_result", "yandex_layout_result", "strict_result"]) {
        if (Object.prototype.hasOwnProperty.call(source, key)) target[key] = source[key];
      }
    }
    ...
    function emitPersonalImageClassificationDiagnostic(logger, diagnostic) {
      ...
      emitPersonalImagePipelineTelemetry(logger, diagnostic);
    }
    function personalImageTriState(value, observed) {
      if (!observed) return "unknown";
      return value === true ? "true" : value === false ? "false" : "unknown";
    }
    function safePersonalImagePipelineTelemetryPayload(diagnostic) {
      const source = diagnostic && typeof diagnostic === "object" ? diagnostic : {};
      const safe = {
        source_type: personalImageDiagnosticEnum(source.source_type, ["original", "preview", "preview_fallback", "unknown"]),
        extension_enum: personalImageDiagnosticEnum(source.extension_enum, ["jpg", "jpeg", "png", "webp", "heic", "heif", "unknown"]),
        declared_mime_enum: personalImageDiagnosticEnum(source.declared_mime_enum, ["jpeg", "png", "webp", "heic", "heif", "unknown"]),
        detected_mime_by_magic_bytes: personalImageDiagnosticEnum(source.detected_mime_by_magic_bytes, ["jpeg", "png", "webp", "heic", "heif", "unknown"]),
        mime_match: source.mime_match === true ? "true" : source.mime_match === false ? "false" : "unknown",
        size_bucket: personalImageDiagnosticEnum(source.size_bucket, ["<250kb", "250kb-1mb", "1-5mb", ">5mb"], "<250kb"),
        primary_cache: personalImageDiagnosticEnum(source.primary_cache, ["hit", "miss"], "miss"),
        dedicated_cache: personalImageDiagnosticEnum(source.dedicated_cache, ["hit", "miss"], "miss"),
        strict_cache: personalImageDiagnosticEnum(source.strict_cache, ["hit", "miss"], "miss"),
        primary_transport: personalImageTransportEnum(source.primary_transport),
        primary_parser: personalImageParserEnum(source.primary_parser),
        primary_normalized_result: personalImageDiagnosticEnum(source.primary_normalized_result, ["work_photo", "receipt", "document", "mailing", "unknown"]),
        dedicated_transport: personalImageTransportEnum(source.dedicated_transport),
        dedicated_parser: personalImageParserEnum(source.dedicated_parser),
        dedicated_normalized_result: personalImageDiagnosticEnum(source.dedicated_normalized_result, ["work", "document", "unknown"]),
        dedicated_has_visible_client: personalImageTriState(source.dedicated_has_visible_client, source._dedicated_schema_observed === true),
        dedicated_has_visible_service_area: personalImageTriState(source.dedicated_has_visible_service_area, source._dedicated_schema_observed === true),
        dedicated_kind: personalImageDiagnosticEnum(source.dedicated_kind, ["hair", "nails", "pedicure", "brows_lashes", "unknown"]),
        dedicated_document_block: source.dedicated_document_block === true ? "true" : "false",
        dedicated_banking_block: source.dedicated_banking_block === true ? "true" : "false",
        receipt_openai_transport: personalImageTransportEnum(source.receipt_openai_transport),
        receipt_openai_parser: personalImageParserEnum(source.receipt_openai_parser),
    ...
    function emitPersonalImagePipelineTelemetry(logger, diagnostic) {
      if (!logger || typeof logger.info !== "function") return;
      const safe = safePersonalImagePipelineTelemetryPayload(diagnostic);
    ...
    function receiptStageContext(file, content, config) {
      ...
      const uploadId = String(file && (file._id || file.id) || "").trim();
      const cached = evidenceKey ? primaryReceiptEvidenceCache.get(evidenceKey) : void 0;
      const fresh = cached && Date.now() - Number(cached.createdAt || 0) < 10 * 60 * 1e3;
      return {
        evidenceKey,
        caseToken: uploadId ? sha256Bytes(utf8Bytes(`receipt-stage-v1:${uploadId}`)).slice(0, 16) : "",
        primaryOpenAi: fresh ? { ...cached.candidate } : void 0,
        primaryProviderGroup: fresh ? "openai" : "",
        primaryPassType: fresh ? "primary" : "",
        primaryConsumed: false
      };
    }
    function rememberPrimaryReceiptEvidence(file, content, config, candidate) {
      const evidenceKey = receiptPrimaryEvidenceKey(file, content, config);
      if (!evidenceKey || !candidate || String(candidate.receiptAmountSource || "").indexOf("openai:") !== 0) return;
      primaryReceiptEvidenceCache.set(evidenceKey, { createdAt: Date.now(), candidate: { ...candidate } });
      if (primaryReceiptEvidenceCache.size > 200) primaryReceiptEvidenceCache.delete(primaryReceiptEvidenceCache.keys().next().value);
    }
    function takeReusablePrimaryReceiptEvidence(file, content, config, context) {
      if (!context || context.primaryConsumed || !context.primaryOpenAi) return void 0;
      const expectedKey = receiptPrimaryEvidenceKey(file, content, config);
      if (!expectedKey || context.evidenceKey !== expectedKey) return void 0;
      if (context.primaryProviderGroup !== "openai" || context.primaryPassType !== "primary") return void 0;
      const candidate = context.primaryOpenAi;
      if (!candidate || String(candidate.receiptAmountSource || "").indexOf("openai:") !== 0) return void 0;
      // A negative/inconclusive classification is not sufficient evidence for
      // financial validation. Preserve the legacy primary request in that
      // case; only a structured candidate that actually marks a receipt can
      // safely replace the identical second primary analysis.
      if (!aiCandidateMarksReceipt(candidate)) return void 0;
    ...
    function logReceiptStage(logger, context, stage, startedAt, outcome = "ok") {
      ...
      logger.info(`RECEIPT_STAGE case=${context.caseToken} stage=${stage} elapsedMs=${elapsedMs} outcome=${outcome}`);
    }
    function logReceiptMediaResolved(message, logger) {
      const file = messageImageFiles(message)[0];
      const uploadId = String(file && (file._id || file.id) || "").trim();
      if (!uploadId || !logger) return;
    ...
    function receiptAmountHasIndependentConfirmation(candidates, selected, requiredDate, allowOpenAiPair = true) {
      ...
      return (Array.isArray(candidates) ? candidates : []).some((candidate) => {
        if (!candidate || candidate === selected || candidate.combinedReceipt) return false;
        if (candidate.receiptDate !== requiredDate || !isValidReceiptAmount(candidate.receiptAmount)) return false;
    ...
    function looksLikeMailingProofText(text) {
      ...
      return Boolean(
        ...
        repeatedStates && (directUi || accountList)
      );
    }
    ...
    async function validateReceiptDate(file, content, http, config, logger, retryAttempt = 0, validationContext, diagnostic) {
      ...
      try {
        ...
        if (hasOpenAi) {
          try {
            let aiCandidate = takeReusablePrimaryReceiptEvidence(file, content, config, stageContext);
            if (aiCandidate) {
              const reusedParsed = parseReceiptJson(aiCandidate.text);
              captureOpenAiReceiptTelemetry(diagnostic, "receipt", "2xx", openAiReceiptParserState(aiCandidate.text, reusedParsed), reusedParsed, aiCandidate);
              logReceiptStage(logger, stageContext, "primary_openai_reused");
            } else {
              const primaryStartedAt = Date.now();
              logReceiptStage(logger, stageContext, "primary_openai_requested");
              aiCandidate = await requestOpenAiReceiptCheck(file, content, http, config, requiredDate, logger, 0, false, false, diagnostic, "receipt");
        ...
        if (errorText.indexOf("OCR HTTP 429") !== -1 && retryAttempt < 2) {
          ...
          return validateReceiptDate(file, content, http, config, logger, retryAttempt + 1, stageContext, diagnostic);
        }
    ...
    async function validateReceiptStrict(file, content, http, config, logger, validationContext, diagnostic) {
      ...
      if (!content || !content.length) {
        capturePersonalImageCacheTelemetry(diagnostic, "strict_cache", false);
      ...
      const promise = (async () => {
        const first = await validateReceiptDate(file, content, http, config, logger, 0, stageContext, diagnostic);
        ...
        const retried = await validateReceiptDate(file, content, http, config, logger, 0, stageContext, diagnostic);
        return retried && retried.ok ? retried : first;
      })();
      strictReceiptValidationCache.set(key, { createdAt: Date.now(), promise, diagnostic });
          ...
          for (const messageFile of files) {
            ...
            if (receiptEntry) {
              try {
                ...
                if (!receiptEntry.archiveKey || receiptEntry.archiveStatus !== "stored" || !await archiveUploadExists(receiptEntry, read) || archiveConfigured(config) && (receiptEntry.externalArchiveStatus !== "stored" || !receiptEntry.externalArchiveKey)) {
    ...
    const RECEIPT_SOURCE_CHAT_CLEANUP_ENABLED = true;
    async function cleanupArchivedReceiptMessages(room, read, persistence, modify, logger, config, http) {
      ...
      for (const entry of index.photos) {
        ...
        try {
          ...
          if (!entry.archiveKey || entry.archiveStatus !== "stored" || !await archiveUploadExists(entry, read) || archiveConfigured(config) && (entry.externalArchiveStatus !== "stored" || !entry.externalArchiveKey)) {
          ...
          const sourceCreatedAt = Number(entry.uploadedAt || message.createdAt && new Date(message.createdAt).getTime() || 0);
          if (!sourceCreatedAt || sourceCreatedAt + RECEIPT_ARCHIVE_RETENTION_MS > now) continue;
          await deleteReceiptMessage(message, read, modify, logger);
          entry.chatDeletedAt = Date.now();
        ...
        for (const oldMessage of messages) {
          ...
          for (const messageFile of files) {
            ...
            try {
              ...
              if (!entry.archiveKey || entry.archiveStatus !== "stored") continue;
              if (!RECEIPT_SOURCE_CHAT_CLEANUP_ENABLED) continue;
              if (createdAt + RECEIPT_ARCHIVE_RETENTION_MS > now) continue;
    ...
    async function cleanupExpiredReceiptArchive(read, persistence, modify, logger, config) {
      if (!read || !persistence || !modify) return 0;
      ...
      for (const entry of index.photos) {
        ...
        if (!archiveConfigured(config) || entry.externalArchiveStatus !== "stored" || !entry.externalArchiveKey) {
          if (logger) logger.warn(`Keeping receipt archive metadata ${entry.exact || "unknown"}: Yandex backup is not confirmed`);
          continue;
        }
      ...
      if (changed) await writeIndex(persistence, PROTECTED_ROOMS.kassa.index, index);
      if (changed && logger) logger.info(`Expired ${Object.keys(expiredDates).length} receipt archive day(s) after confirmed Yandex backup`);
      return 0;
    }
    ...
    function receiptOwnerUsernameForRoom(room) {
      const slug = String(room && room.slugifiedName || "").toLowerCase();
    ...
    async function receiptOwnerForMessage(message, read, fallbackToSender = true) {
      ...
      if (username) {
        try {
          const owner = await read.getUserReader().getByUsername(username);
          if (owner) return owner;
        } catch (_7) {
        }
      }
      if (message && message.room && isPersonalTarsRoom(message.room)) {
        try {
          const members = await read.getRoomReader().getMembers(message.room.id);
          const owners = (members || []).filter((member) => {
            const candidate = String(member && member.username || "").replace(/^@/, "").trim().toLowerCase();
            return candidate && candidate !== "tars" && candidate !== "teimur" && candidate !== "shura";
          });
    ...
    async function publishAcceptedReceipt(entry, message, read, modify, config, logger) {
      ...
      const receiptOwner = await receiptOwnerForMessage(message, read) || message.sender;
      const username = receiptOwner.username ? `@${receiptOwner.username}` : receiptOwner.name || receiptOwner.id;
    ...
    async function guardUpload(context, read, persistence, modify, logger, http, ocrConfig) {
      ...
      if (isPersonalTarsRoom(room)) {
        ...
        // Never call OCR or Vision from the pre-upload hook for a master's
        // personal room. Rocket.Chat mobile keeps the file in an "upload in
        // progress" state while this hook is running and may abort the upload
        // before the external classifier returns. The post-message handler has
        // the complete uploaded file and performs the same receipt/photo/
        // mailing routing there without blocking the client.
        return;
      }
    ...
    async function notifyWorkPhotoAccepted(message, read, modify) {
      const appUser = await read.getUserReader().getByUsername("tars") || await read.getUserReader().getAppUser();
      if (!appUser || !message || !message.room) return false;
    ...
    async function rejectDuplicateMessage(message, read, persistence, modify, logger, http, ocrConfig, forcedIntent = "", processingStatusManager, personalImageDiagnostic) {
      ...
      const intent = forcedIntent || directFileIntent(message);
      ...
      for (const messageFile of imageFiles) {
        try {
          ...
          let content;
          let uploadReadError;
          // Rocket.Chat can emit PostMessageSent while the mobile upload is
          // already visible in the room but not yet readable by UploadReader.
          // Photos already wait for this settle window; receipts need the same
          // protection or their only processing attempt is silently lost.
          for (let attempt = 0; attempt < 8 && !content; attempt += 1) {
            if (attempt > 0) await new Promise((resolve) => setTimeout(resolve, 1e3));
            try {
              content = await read.getUploadReader().getBufferById(messageFileId);
            } catch (error) {
              uploadReadError = error;
              if (logger) logger.warn(`POST_UPLOAD_NOT_READY attempt=${attempt + 1} upload=${messageFileId}: ${error && error.message || error}`);
            }
          }
          if (!content || !content.length) {
          ...
          if (isCurrentPreUpload) {
            if (protectedRoom.kind === "receipt") {
              ...
              } : prevalidatedReceiptCheck || await validateReceiptStrict(messageFile, content, http, ocrConfig, logger, receiptValidationContext, personalImageDiagnostic);
              if (!receiptCheck.ok) {
      ...
      if (!duplicate) {
        ...
        if (receiptEntries.length) {
          ...
          for (const entry of receiptEntries) {
            ...
            refreshed[key] = true;
            try {
              await publishMasterTransferSummary(entry, message, read, persistence, modify, ocrConfig, logger, true, receiptEntries);
              if (processingStatusManager) processingStatusManager.markAll("running_total_published", "ok");
            } catch (error) {
              if (logger) logger.warn(`Could not refresh master transfer summary: ${error && error.message || error}`);
            }
          }
        }
        const photoEntries = Object.keys(acceptedByIndex).reduce((list, indexName) => {
          const roomConfig = roomCache[indexName];
    ...
    async function processPersonalMediaV2(message, read, persistence, modify, logger, http, ocrConfig, forcedIntent = "", allowProcessingStatus = true) {
      ...
      // receipts/documents/mailings, guards duplicates and reuses the original
      // Rocket.Chat upload. This path must run before the financial pipeline:
      ...
      try {
        ...
        if (handled) {
          const finalReason = personalImageDiagnostic.final_reason === "unknown" ? personalImageDiagnostic.strict_result !== "not_reached" ? "strict-receipt-check" : "receipt" : personalImageDiagnostic.final_reason;
          setPersonalImageFinalDiagnostic(personalImageDiagnostic, "receipt", finalReason, "final");
          emitPersonalImageClassificationDiagnostic(logger, personalImageDiagnostic);
        }
    ...
    async function confirmedTransferSummaryForUser(read, config, userId, targetDate, nameCandidates, currentValidatedReceipts, sourceRoomId = "") {
      ...
      let pendingReview = 0;
      for (const entry of index.photos) {
        if (!entry || entry.source !== "rejected") continue;
      ...
      // The protected receipt index is the source of truth. A derived ledger
      // can be incomplete after an update or stale after manual rejection, so
      // never short-circuit the full index reconciliation here.
      ...
      return { count, total: Math.round(total * 100) / 100, missing, pendingReview };
    }
    ...
    function masterTransferLedgerAssociation(userId, targetDate) {
      return new RocketChatAssociationRecord(
        RocketChatAssociationModel.MISC,
        `receipt-transfer-ledger-v1:${targetDate}:${userId}`
      );
    }
    function receiptLedgerEntryKey(receipt) {
      const exact = String(receipt && receipt.exact || "");
    ...
    async function receiptLedgerSummaryForUser(entry, currentValidatedReceipts, read, persistence, config, sourceRoomId = "") {
      ...
      // The receipt index is authoritative. Rebuild the derived ledger every
      // time so an app update, a rejected receipt or a failed earlier write
      // cannot leave the daily total incomplete or stale.
      await persistence.removeByAssociation(association);
      for (const [receiptKey, amount] of rows.entries()) {
        await persistence.createWithAssociation({ receiptKey, amount, userId, targetDate, createdAt: Date.now() }, association);
      }
      const amounts = Array.from(rows.values());
      const total = amounts.reduce((sum, amount) => sum + amount, 0);
      return { count: amounts.length, amounts, total: Math.round(total * 100) / 100, missing };
    }
    let receiptSummaryQueue = Promise.resolve();
    async function publishMasterTransferSummary(entry, message, read, persistence, modify, config, logger, includeCurrentValidatedReceipt = false, currentValidatedReceipts) {
      const runSummary = async () => {
        return publishMasterTransferSummaryUnlocked(entry, message, read, persistence, modify, config, logger, includeCurrentValidatedReceipt, currentValidatedReceipts);
      };
      const result = receiptSummaryQueue.then(runSummary, runSummary);
      receiptSummaryQueue = result.then(() => void 0, () => void 0);
      return result;
    }
    async function publishMasterTransferSummaryUnlocked(entry, message, read, persistence, modify, config, logger, includeCurrentValidatedReceipt = false, currentValidatedReceipts) {
      const receiptOwner = await receiptOwnerForMessage(message, read);
      const ownerEntry = receiptOwner ? { ...entry, userId: receiptOwner.id, username: receiptOwner.username || "", userName: receiptOwner.name || "" } : entry;
      const ownerReceipts = (Array.isArray(currentValidatedReceipts) ? currentValidatedReceipts : currentValidatedReceipts ? [currentValidatedReceipts] : entry ? [entry] : []).map((receipt) => receiptOwner ? { ...receipt, userId: receiptOwner.id, username: receiptOwner.username || "", userName: receiptOwner.name || "" } : receipt);
      const userId = String(ownerEntry && ownerEntry.userId || "");
      const nameCandidates = ownerEntry && Array.isArray(ownerEntry.nameCandidates) ? ownerEntry.nameCandidates : [];
      const associationKey = userId || "name:" + transferNameKey(ownerEntry && (ownerEntry.username || ownerEntry.userName || nameCandidates[0]) || "");
      if (!associationKey || associationKey === "name:") return false;
      const targetDate = String(ownerEntry && ownerEntry.receiptDate || expectedReceiptDate(config));
      const room = message && isPersonalTarsRoom(message.room) ? message.room : await findResultRoom(read, config);
      const appUser = await read.getUserReader().getByUsername("tars") || await read.getUserReader().getAppUser();
      const sourceRoomId = String(message && message.room && message.room.id || ownerEntry && ownerEntry.roomId || "");
      const summary = await receiptLedgerSummaryForUser(ownerEntry, includeCurrentValidatedReceipt ? ownerReceipts : void 0, read, persistence, config, sourceRoomId);
      ...
      const ledgerLines = summary.amounts.map((amount, index) => `${index + 1}. ${new Intl.NumberFormat("ru-RU", { minimumFractionDigits: 0, maximumFractionDigits: 2 }).format(amount)} ₽`).join("\n");
      let text = `🧾 ИТОГО ПО ЧЕКАМ ЗА ${displayDate(targetDate)}\n${ledgerLines}${ledgerLines ? "\n\n" : ""}Чеков: ${summary.count}\nОбщая сумма чеков: ${amountText} ₽`;
    ...
    async function seedExistingPhotos(read, persistence, logger) {
      await seedRoom(read, persistence, logger, "result", PROTECTED_ROOMS.otchet);
      await seedRoom(read, persistence, logger, "Результат", PROTECTED_ROOMS.otchet);
    }
    module2.exports = {
      exactHash,
      expectedReceiptDate,
      ...
      messageImageFiles,
      messageLooksLikePendingImageUpload,
      resolvePersonalImageMessageV2,
      logReceiptMediaResolved,
      ...
      personalImageExtensionEnum,
      personalImageDeclaredMimeEnum,
      detectedPersonalImageMimeByMagicBytes,
      personalImageSizeBucket,
...
var C = class extends j.App {
  ...
  async onUpdate(e, n, t, s, r) {
    ...
    try {
      await this.ensureCashRoomName(n, r);
      // Refresh personal menus first. Later photo/archive maintenance may fail
      // independently and must never leave duplicate report buttons behind.
      await this.refreshKnownPersonalReportRooms(n, r, s);
      await G.cleanupDuplicateReportForwardsInOtchet(n, r, this.getLogger());
  ...
  async checkPostMessageSent(e, n, t) {
    ...
    let eventRoom = e.room;
    // Mobile upload events can expose only room.id during the pre-attachment
    // message event. Hydrate the room before deciding whether this is a TARS
    // personal channel; otherwise Apps-Engine never calls executePostMessageSent.
    if (!G.isPersonalTarsRoom(eventRoom) && eventRoom && eventRoom.id) {
      try {
        eventRoom = await n.getRoomReader().getById(eventRoom.id) || eventRoom;
      } catch (roomError) {
  ...
  async executePostMessageSent(e, n, t, s, r) {
    if (e && e.room && e.room.id && !G.isPersonalTarsRoom(e.room)) {
      try {
        ...
        if (hydratedRoom && G.isPersonalTarsRoom(hydratedRoom)) e = { ...e, room: hydratedRoom };
      } catch (roomError) {
    ...
    const rememberUploadId = (file) => {
      const titleLink = file && file.title && typeof file.title === "object" ? file.title.link : "";
      const uploadId = String(file && (file._id || file.id || file.url || file.path || titleLink || file.name) || "").split("?")[0];
    ...
    // On mobile Rocket.Chat commonly emits the settled upload only through
    // attachments (title.link/imageUrl), without populating file/files. Include
    // those parsed image ids in the event key so the preliminary message event
    // cannot suppress the real receipt/photo event as a duplicate.
    for (const file of G.messageImageFiles(e)) rememberUploadId(file);
    ...
    // Rocket.Chat can emit the same message twice: first before the mobile
    // upload is attached, then again with the real upload id. A message-only
    // event must not suppress the later file event, otherwise automatic photo
    // and receipt handling only works after the user presses a button.
    const localSeen = Boolean(messageId && !uploadEventKey && !G.messageImageFiles(e).length && recentPostMessageIds.has(messageId));
    ...
    if (localSeen || localUploadSeen) {
      ...
      try {
        ...
        if (explicitMailingIntent) {
          ...
          const mailingUploadId = String(mailingFile._id || mailingFile.id || mailingFile.name || messageId || "");
          const existingMailings = await n.getPersistenceReader().readByAssociation(this.mailingProofIndexAssociation(mailingWorkday));
          if (!(existingMailings || []).some((entry) => entry && entry.roomId === e.room.id && String(entry.uploadId || "") === mailingUploadId)) {
            await s.createWithAssociation({
              ...
              roomId: e.room.id,
              uploadId: mailingUploadId,
              workday: mailingWorkday,
              source: "mailing-report-button",
              createdAt: Date.now()
            }, this.mailingProofIndexAssociation(mailingWorkday));
          }
          await this.clearMailingReportIntent(s, e.room);
          const appUser = await n.getUserReader().getByUsername("tars") || await n.getUserReader().getAppUser();
      ...
      if (uploadEventKey && !postMessageClaimToken) {
        ...
        if (!postMessageClaimToken) {
          ...
          // Rocket.Chat persistence readers can lag behind the claim write on
          // a fresh upload. Never drop a personal image because the claim is
          // temporarily invisible: rejectDuplicateMessage is independently
          // idempotent by message id, upload id and content fingerprint.
          if (!hasPersonalImageUpload) return;
      ...
      if (messageId) recentPostMessageIds.delete(messageId);
      if (uploadEventKey) recentPostUploadIds.delete(uploadEventKey);
  ...
  mailingReportIntentAssociation(roomId) {
    return new y.RocketChatAssociationRecord(y.RocketChatAssociationModel.MISC, `mailing-report-intent:${roomId}`);
  }
  transferReportIntentAssociation(roomId) {
    return new y.RocketChatAssociationRecord(y.RocketChatAssociationModel.MISC, `transfer-report-intent:${roomId}`);
  }
  async activePhotoReportIntent(read, room) {
    if (!read || !room || !room.id) return false;
    const records = await read.getPersistenceReader().readByAssociation(this.photoReportIntentAssociation(room.id));
    return (records || []).some((entry) => entry && Number(entry.expiresAt || 0) > Date.now());
  }
  async clearPhotoReportIntent(persistence, room) {
    if (persistence && room && room.id) await persistence.removeByAssociation(this.photoReportIntentAssociation(room.id));
  }
  async activeMailingReportIntent(read, room) {
    if (!read || !room || !room.id) return false;
    const records = await read.getPersistenceReader().readByAssociation(this.mailingReportIntentAssociation(room.id));
    return (records || []).some((entry) => entry && Number(entry.expiresAt || 0) > Date.now());
  }
  async clearMailingReportIntent(persistence, room) {
    if (persistence && room && room.id) await persistence.removeByAssociation(this.mailingReportIntentAssociation(room.id));
  }
  async activeTransferReportIntent(read, room) {
    if (!read || !room || !room.id) return false;
    const records = await read.getPersistenceReader().readByAssociation(this.transferReportIntentAssociation(room.id));
    return (records || []).some((entry) => entry && Number(entry.expiresAt || 0) > Date.now());
  }
  ...
  async removeUploadTypeMenu(modify, data) {
    ...
    try {
      ...
      this.getLogger().warn(`Could not remove upload type menu: ${error && error.message || error}`);
    }
  }
  async handleUploadMenuButton(read, modify, data) {
    if (!read || !modify || !data || !data.room || !this.isPersonalReportRoom(data.room)) return;
    const appUser = await read.getUserReader().getByUsername("tars") || await read.getUserReader().getAppUser();
    if (!appUser) return;
    try {
      const recent = await read.getRoomReader().getMessages(data.room.id, { limit: 30, sort: { createdAt: "desc" }, showThreadMessages: false });
      for (const message of recent || []) {
        if (String(message && message.sender && message.sender.id || "") !== String(appUser.id || "")) continue;
        if (String(message && message.text || "") !== "ВЫБЕРИТЕ ТИП ЗАГРУЗКИ") continue;
        await modify.getDeleter().deleteMessage(message, message.sender);
      }
    } catch (error) {
      this.getLogger().warn(`Could not remove previous upload type menu: ${error && error.message || error}`);
    }
    const blocks = modify.getCreator().getBlockBuilder();
    blocks.addSectionBlock({ text: blocks.newMarkdownTextObject("Выберите, что хотите загрузить:") });
    ...
    await modify.getCreator().finish(
      modify.getCreator().startMessage().setSender(appUser).setRoom(data.room).setText("ВЫБЕРИТЕ ТИП ЗАГРУЗКИ").setBlocks(blocks)
    );
  }
  async handlePhotoReportButton(read, modify, persistence, data) {
    if (!read || !modify || !persistence || !data || !data.room || !data.user || !this.isPersonalReportRoom(data.room)) return;
    await this.clearMailingReportIntent(persistence, data.room);
    await this.clearTransferReportIntent(persistence, data.room);
    const association = this.photoReportIntentAssociation(data.room.id);
    await persistence.removeByAssociation(association);
    await persistence.createWithAssociation({
      roomId: data.room.id,
    ...
    if (appUser) {
      await modify.getCreator().finish(
        modify.getCreator().startMessage().setSender(appUser).setRoom(data.room).setText("📸 Отправьте фото выполненной работы.")
      );
    }
    await this.removeUploadTypeMenu(modify, data);
  }
  async handleReceiptUploadButton(read, modify, persistence, data) {
    if (!read || !modify || !persistence || !data || !data.room || !this.isPersonalReportRoom(data.room)) return;
    await this.clearPhotoReportIntent(persistence, data.room);
    await this.clearMailingReportIntent(persistence, data.room);
    const association = this.transferReportIntentAssociation(data.room.id);
    await persistence.removeByAssociation(association);
    await persistence.createWithAssociation({
      roomId: data.room.id,
      requestedBy: data.user && data.user.id || "",
      createdAt: Date.now(),
      expiresAt: Date.now() + 10 * 60 * 1e3
    }, association);
    const appUser = await read.getUserReader().getByUsername("tars") || await read.getUserReader().getAppUser();
  ...
  async handleMailingUploadButton(read, modify, persistence, data) {
    if (!read || !modify || !persistence || !data || !data.room || !this.isPersonalReportRoom(data.room)) return;
    await this.clearPhotoReportIntent(persistence, data.room);
    await this.clearTransferReportIntent(persistence, data.room);
    const association = this.mailingReportIntentAssociation(data.room.id);
    await persistence.removeByAssociation(association);
    await persistence.createWithAssociation({
      roomId: data.room.id,
      requestedBy: data.user && data.user.id || "",
    ...
    const appUser = await read.getUserReader().getByUsername("tars") || await read.getUserReader().getAppUser();
    if (appUser) await modify.getCreator().finish(
      modify.getCreator().startMessage().setSender(appUser).setRoom(data.room).setText("✉️ Отправьте скриншот рассылки.")
    );
    await this.removeUploadTypeMenu(modify, data);
  }
  async receiptWasAcceptedForMessage(read, message) {
    if (!read || !message) return false;
    const messageId = String(message.id || "");
    const uploadIds = [];
    const remember = (file) => {
      const id = String(file && (file._id || file.id) || "");
      if (id && uploadIds.indexOf(id) === -1) uploadIds.push(id);
    };
  ...
  async sendUnknownImageTypePrompt(read, modify, message) {
    if (!read || !modify || !message || !message.room) return;
    const appUser = await read.getUserReader().getByUsername("tars") || await read.getUserReader().getAppUser();
    if (!appUser) return;
    await modify.getCreator().finish(
      modify.getCreator().startMessage().setSender(appUser).setRoom(message.room).setText("⚠️ Изображение не принято: Тарс не смог уверенно отличить фото работы от чека. Отправьте более чёткое фото без бликов.")
    );
  }
  async executeActionButtonHandler(e, n, t, s, r) {
    ...
    if (a.actionId === PHOTO_REPORT_ACTION) {
      await this.handlePhotoReportButton(n, r, s, a);
      return e.getInteractionResponder().successResponse();
    }
    if (a.actionId === RECEIPT_UPLOAD_ACTION) {
      await this.handleReceiptUploadButton(n, r, s, a);
      return e.getInteractionResponder().successResponse();
    }
    if (a.actionId === MAILING_UPLOAD_ACTION) {
      await this.handleMailingUploadButton(n, r, s, a);
      return e.getInteractionResponder().successResponse();
    }
    if (a.actionId === N) {
      if (a.room) {
  ...
  modalRowsForReportType(e = "male") {
    if (e === "female") return [
      { id: "female_cut_1", label: "Женская стрижка", kind: "service" },
      { id: "female_cut_2", label: "Женская стрижка", kind: "service" },
...
*Итоги:* выручка · зарплата · касса · разница`)
  ...
  async removeLegacyPersonalReportMenus(e, n, t, keepMessageId = "") {
    ...
    try {
      ...
      for (const o of messages) {
        ...
        if (keepMessageId && String(o.id || "") === String(keepMessageId)) continue;
  ...
  async refreshPersonalReportButton(e, n, t, s, r, a = false) {
    const run = () => this.refreshPersonalReportButtonUnlocked(e, n, t, s, r, a);
    const result = personalReportButtonRefreshQueue.then(run, run);
    personalReportButtonRefreshQueue = result.then(() => void 0, () => void 0);
    return result;
  }
  async refreshPersonalReportButtonUnlocked(e, n, t, s, r, a = false) {
  ...
  async finalizeSubmittedReportsJob(e, n, t, s, r) {
    ...
    for (const m of o || []) {
      ...
      try {
        ...
        const E = await this.receiptOcrConfig(n), C = await G.confirmedTransferSummaryForUser(n, E, m.userId, m.workday, void 0, void 0, O.id), A = await this.mailingProofStatus(n, h, m.workday), K = await this.reportPhotoStatus(n, h, m.workday), M = f.timeCorrection || this.reportTimeCorrection(f.firstSubmittedAt || f.updatedAt || c, f.scheduleStatus || null);
  ...
  async resolveLatenessUser(e, n, t) {
    ...
    await o(n);
    for (const c of ["general", "General", "result", "Result", "Результат", "результат", "Otchet", "otchet", "cheki-kontrol", "cheki_arhiv", "rasblLki"]) {
      try {
  ...
  async reportPhotoStatus(e, n, t) {
    ...
    let o = 0;
    const c = {};
    const directRoomIds = new Set(r.filter((entry) => entry.direct).map((entry) => String(entry.room.id)));
    try {
      const photoIndex = await G.readIndex(e, G.PROTECTED_ROOMS.otchet.index);
      for (const entry of photoIndex && Array.isArray(photoIndex.photos) ? photoIndex.photos : []) {
        if (!entry || !directRoomIds.has(String(entry.roomId || ""))) continue;
        const published = Boolean(entry.reportUploadId || entry.reportMessageId && !["publishing", "duplicate", "blocked", "failed"].includes(String(entry.reportMessageId)));
        if (!published) continue;
        const photoWorkday = this.reportWorkday(new Date(Number(entry.uploadedAt || entry.reportPublishedAt || 0)));
        if (t && photoWorkday !== t) continue;
        const key = String(entry.reportMessageId || entry.reportUploadId || entry.uploadId || entry.exact || "");
        if (!key || c[key]) continue;
        c[key] = true;
        o += 1;
      }
    } catch (photoIndexError) {
      this.getLogger().warn(`Could not read accepted photo index for report status: ${photoIndexError && photoIndexError.message || photoIndexError}`);
    }
    for (const d of r) {
      for (let m = 0; m < 3; m += 1) {
  ...
  async cleanupPrivateCashRoomsJob(e, n, t, s, r) {
    ...
    for (let o of records || []) {
      ...
      try {
        ...
        if (c) {
          ...
          if (config.archiveEnabled && r) a += await G.cleanupArchivedReceiptMessages(c, n, r, t, this.getLogger(), config, s);
        }
      } catch (c) {
    ...
    if (config.archiveEnabled && r) {
      try {
        a += await G.cleanupExpiredReceiptArchive(n, r, t, this.getLogger(), config);
      } catch (cleanupError) {
    ...
    if (r) {
      try {
        a += await this.cleanupExpiredReportMedia(n, t, r);
      } catch (mediaCleanupError) {
        this.getLogger().warn(`Could not clean expired report media: ${mediaCleanupError && mediaCleanupError.message || mediaCleanupError}`);
      }
    }
    if (a) this.getLogger().info(`Deleted ${a} archived/private messages older than 24 hours`);
  }
  messageUploadIds(message) {
    const files = [];
    if (message && message.file) files.push(message.file);
    if (message && Array.isArray(message.files)) files.push(...message.files);
    return files.map((file) => String(file && (file._id || file.id) || "")).filter(Boolean);
  }
  async deleteReportMediaMessage(read, modify, messageId) {
    if (!messageId) return false;
    const message = await read.getMessageReader().getById(String(messageId));
    if (!message || !this.hasImageMessage(message)) return false;
    await modify.getDeleter().deleteMessage(message, message.sender);
    return true;
  }
  async cleanupExpiredReportMedia(read, modify, persistence) {
    if (!read || !modify || !persistence) return 0;
    const now = Date.now(), cutoff = now - REPORT_MEDIA_RETENTION_MS;
    let deleted = 0;
    const photoIndex = await G.readIndex(read, G.PROTECTED_ROOMS.otchet.index);
    let photoIndexChanged = false;
    for (const entry of photoIndex && Array.isArray(photoIndex.photos) ? photoIndex.photos : []) {
      const mediaTimestamp = Number(entry && (entry.uploadedAt || entry.reportPublishedAt) || 0);
      if (!entry || entry.mediaDeletedAt || !mediaTimestamp || mediaTimestamp > cutoff) continue;
      let deletionFailed = false;
      for (const messageId of [entry.messageId, entry.reportMessageId]) {
        if (!messageId || ["publishing", "duplicate", "blocked", "failed"].includes(String(messageId))) continue;
        try {
          if (await this.deleteReportMediaMessage(read, modify, messageId)) deleted += 1;
        } catch (error) {
          deletionFailed = true;
          this.getLogger().warn(`Could not delete expired work photo ${messageId}: ${error && error.message || error}`);
        }
      }
      if (deletionFailed) continue;
      entry.mediaDeletedAt = now;
      photoIndexChanged = true;
    }
    if (photoIndexChanged) await G.writeIndex(persistence, G.PROTECTED_ROOMS.otchet.index, photoIndex);

    for (let daysAgo = 60; daysAgo <= 90; daysAgo += 1) {
      const workday = this.reportWorkday(new Date(now - daysAgo * 24 * 60 * 60 * 1e3));
      const records = await read.getPersistenceReader().readByAssociation(this.mailingProofIndexAssociation(workday));
      const byRoom = new Map();
      for (const record of records || []) {
        if (!record || !record.roomId || !record.uploadId || Number(record.createdAt || 0) > cutoff) continue;
        const marker = new y.RocketChatAssociationRecord(y.RocketChatAssociationModel.MISC, `mailing-media-cleaned:${record.uploadId}`);
        if ((await read.getPersistenceReader().readByAssociation(marker) || []).length) continue;
        if (!byRoom.has(record.roomId)) byRoom.set(record.roomId, []);
        byRoom.get(record.roomId).push({ record, marker });
      }
      for (const [roomId, pending] of byRoom) {
        const wanted = new Map(pending.map((item) => [String(item.record.uploadId), item]));
        for (let skip = 0; skip < 1e3 && wanted.size; skip += 100) {
          const messages = await read.getRoomReader().getMessages(roomId, { limit: 100, skip, sort: { createdAt: "desc" }, showThreadMessages: false });
          if (!messages || !messages.length) break;
          for (const message of messages) {
            for (const uploadId of this.messageUploadIds(message)) {
              const item = wanted.get(uploadId);
              if (!item) continue;
              try {
                if (await this.deleteReportMediaMessage(read, modify, message.id)) deleted += 1;
                await persistence.createWithAssociation({ uploadId, roomId, workday, deletedAt: now }, item.marker);
                wanted.delete(uploadId);
              } catch (error) {
                this.getLogger().warn(`Could not delete expired mailing screenshot ${uploadId}: ${error && error.message || error}`);
              }
            }
          }
          if (messages.length < 100) break;
  ...
  async getPublicReportRoom(e) {
    let n = e.getRoomReader();
    const resultRoom = await n.getByName("result") || await n.getByName("Result") || await n.getByName("Результат") || await n.getByName("результат");
  ...
  async sendPersonalReportLink(e, n, t, s, r, a = "male") {
    ...
    const tokenRecord = { token: o, userId: r.id, username: r.username || "", userName: r.name || "", roomId: s.id, sourceRoomId: s.id, reportType: a, workday: this.reportWorkday(), expiresAt: Date.now() + D };
    ...
    if (!h) {
      this.getLogger().warn("Report launcher message was not created; keeping the previous one in place");
      return;
    }
    try {
      await this.removeLegacyPersonalReportMenus(e, n, s, h);
    ...
    // Persist the token after menu cleanup so the newly published button
    // cannot be left with a token invalidated by a concurrent refresh.
    await t.createWithAssociation(tokenRecord, c);
  }
  async publishDefaultTable(e, n, t) {
    let s = await this.getPublicReportRoom(e);
    if (!s) {
      this.getLogger().warn(
  ...
  async sendReport(e, n, t, s, r, a, p = "male", i = "", transferVerification = null, reportOwner = null, mailings = 0, mailingProof = null, timeCorrection = null, penalties = null, finalAnalysis = true) {
    ...
    let payroll = this.payrollRule(p, s, mailings, effectiveMailingProof), correction = timeCorrection || { applied: false, amount: 0, label: "👍 до 21:00" }, correctionAmount = correction && correction.applied ? Math.min(300, Number(correction.amount) || 300) : 0, o = s.filter((i) => i.kind === "service").reduce((i, p) => i + p.amount, 0), x = s.filter((i) => i.kind === "service").reduce((i, p) => i + (p.expense || 0), 0), v = o - x, c = s.filter((i) => i.kind === "sale").reduce((i, p) => i + p.amount, 0), tipRows = s.filter((i) => i.kind === "tip"), tipTotal = tipRows.reduce((i, p) => i + p.amount, 0), tipDeduction = tipRows.reduce((i, p) => i + (p.expense || 0), 0), tipSalary = tipRows.reduce((i, p) => i + (p.netAmount || 0), 0), d = o + c + tipTotal, baseSalary = v * payroll.serviceRate + c + tipSalary, m = Math.max(0, baseSalary - correctionAmount), penaltyTotal = Math.max(0, Number(penalties && penalties.total) || 0), penaltyCount = Math.max(0, Number(penalties && penalties.count) || 0), salaryPayable = Math.max(0, m - penaltyTotal), verified = !!transferVerification && Number.isFinite(Number(transferVerification.total)), pendingReceiptCount = Math.max(0, Number(transferVerification && transferVerification.pendingReview) || 0), pendingReceiptReview = pendingReceiptCount > 0, confirmedTransfers = verified ? Number(transferVerification.total) : a, transferDifference = confirmedTransfers - a, u = r + confirmedTransfers, I = u - d, shortage = I < -0.005, surplus = I > 0.005, f = (i) => this.formatRubles(i).replace(" \u20BD", ""), h = (i, p, R = false) => {
      ...
      shortage && pendingReceiptReview ? g("\uD83D\uDFE0 \u041F\u0420\u041E\u0412\u0415\u0420\u041A\u0410 \u0427\u0415\u041A\u041E\u0412", -I) : shortage ? g("\uD83D\uDD34 \u041D\u0415\u0414\u041E\u0421\u0422\u0410\u0427\u0410", -I) : surplus ? g("\uD83D\uDFE1 \u0418\u0417\u041B\u0418\u0428\u0415\u041A", I) : g("\uD83D\uDFE2 \u0421\u041E\u0412\u041F\u0410\u041B\u041E", 0),
      ...
      ...verified ? [
        ...
        pendingReceiptReview ? `\uD83D\uDFE0 \u041E\u0436\u0438\u0434\u0430\u0435\u0442 \u0440\u0443\u0447\u043D\u043E\u0439 \u043F\u0440\u043E\u0432\u0435\u0440\u043A\u0438 \u0447\u0435\u043A\u043E\u0432: *${pendingReceiptCount}*` : "",
      ...
      shortage && pendingReceiptReview ? `\uD83D\uDFE0 \u0418\u0442\u043E\u0433 \u043F\u0440\u0435\u0434\u0432\u0430\u0440\u0438\u0442\u0435\u043B\u044C\u043D\u044B\u0439: \u0447\u0435\u043A\u0438 \u0435\u0449\u0451 \u043F\u0440\u043E\u0432\u0435\u0440\u044F\u044E\u0442\u0441\u044F` : shortage ? `\uD83D\uDD34 \u041D\u0435\u0434\u043E\u0441\u0442\u0430\u0447\u0430: *${this.formatRubles(-I)}*` : surplus ? `\uD83D\uDFE1 \u0418\u0437\u043B\u0438\u0448\u0435\u043A: *${this.formatRubles(I)}*` : "\uD83D\uDFE2 \u041A\u0430\u0441\u0441\u0430 \u0441\u043E\u0432\u043F\u0430\u043B\u0430"
    ].filter(Boolean).join(`
`);
    let k = [{ color: shortage && pendingReceiptReview ? "#f59e0b" : shortage ? "#dc2626" : surplus ? "#f59e0b" : "#16a34a", text: shortage && pendingReceiptReview ? "\uD83D\uDFE0 \u041F\u0420\u041E\u0412\u0415\u0420\u041A\u0410 \u0427\u0415\u041A\u041E\u0412" : shortage ? "\uD83D\uDD34 \u041D\u0415\u0414\u041E\u0421\u0422\u0410\u0427\u0410" : surplus ? "\uD83D\uDFE1 \u0418\u0417\u041B\u0418\u0428\u0415\u041A" : "\uD83D\uDFE2 \u041A\u0410\u0421\u0421\u0410 \u0421\u041E\u0412\u041F\u0410\u041B\u0410" }];
    if (i) {
      try {
  ...
  async sendOwnerShortReport(e, n, reportOwner, rows, cash, transfers, reportType = "male", previousMessageId = "", transferVerification = null, mailings = 0, mailingProof = null, timeCorrection = null, penalties = null, finalAnalysis = true, workday = "") {
    try {
      ...
      if (pendingReceiptReview) violations.push(`чеки на проверке: ${pendingReceiptCount}`);
      ...
      if (cashDifference < -0.005 && !pendingReceiptReview) violations.push(`недостача: ${this.formatRubles(-cashDifference)}`);
      else if (cashDifference > 0.005) violations.push(`излишек: ${this.formatRubles(cashDifference)}`);
      const masterUsername = String(reportOwner && reportOwner.username || "").replace(/^@/, ""), masterDisplayName = String(reportOwner && reportOwner.name || masterUsername || "Мастер").trim(), masterLabel = masterUsername && masterUsername.toLowerCase() !== masterDisplayName.toLowerCase() ? `${masterDisplayName} (@${masterUsername})` : masterUsername ? `@${masterUsername}` : masterDisplayName, date = this.displayWorkday(workday || this.reportWorkday()), text = [`📊 *ОТЧЁТ МАСТЕРА*`, `Мастер: *${masterLabel}*`, `Дата: *${date}*`, `Клиентов: *${clientCount}*`, `Зарплата: *${this.formatRubles(salaryPayable)}*`, `Нарушения: ${violations.length ? violations.join("; ") : "нет ✅"}`].join("\n");
  ...
  defaultFemaleExpense(e, n) {
    ...
    return t.includes("\u043E\u043A\u0440\u0430\u0448") ? 2e3 * s : null;
  }
...
var S = class extends A.ApiEndpoint {
  ...
  async get(e, n, t, s, r, a) {
    ...
    if (!tokenRoom || !this.reportApp.isPersonalReportRoom(tokenRoom) || tokenMaster && String(tokenMaster.id || "") !== String(m.userId || "")) {
  ...
  async post(e, n, t, s, r, a) {
    ...
    const tokenMaster = tokenRoom && this.reportApp.isPersonalReportRoom(tokenRoom) ? await this.reportApp.masterUserForPersonalReportRoom(t, tokenRoom) : void 0;
    if (!tokenRoom || !this.reportApp.isPersonalReportRoom(tokenRoom) || tokenMaster && String(tokenMaster.id || "") !== String(u.userId || "")) {
...
var REPORT_ON_TIME_IMAGE_DATA = "data:image/jpeg;base64,";
var REPORT_LATE_IMAGE_DATA = "data:image/jpeg;base64,";
REPORT_FORM_SCRIPT = REPORT_FORM_SCRIPT.replace(
  "female:{title:'Отчёт женского мастера',rows:[['Женская стрижка'],['Укладка']",
  "female:{title:'Отчёт женского мастера',rows:[['Женская стрижка'],['Женская стрижка'],['Укладка']"
);
REPORT_FORM_SCRIPT = REPORT_FORM_SCRIPT.replace(
  "reportType==='male'&&rowName.indexOf('окраш')!==-1?400*qty:0",
  "reportType==='female'&&rowName.indexOf('окраш')!==-1?2000*qty:reportType==='male'&&rowName.indexOf('окраш')!==-1?400*qty:0"
);
var REPORT_ON_TIME_SUCCESS_OVERRIDE = `
    showAccepted = function(data) {
      ...
      const styles = '<style id="acceptedStyles">#acceptedOverlay{position:fixed;inset:0;z-index:9999;background:#050505;padding:0;display:grid;place-items:center;overflow:hidden;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace}#acceptedCard{width:100vw;height:100vh;height:100dvh;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#050505}#acceptedImage{display:block;width:100%;height:100%;object-fit:cover;object-position:center}</style>';
      if (!late) {
