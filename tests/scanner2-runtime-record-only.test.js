...
(async () => {
  ...
  assert.match(source, /id:\s*"scanner2_shadow_retention_days"[\s\S]*packageValue:\s*"30"/);
  ...
  assert.strictEqual(appManifest.version, "0.10.26");
  assert.strictEqual(packageManifest.version, "0.10.26");
  assert.strictEqual(packageLock.version, "0.10.26");
  assert.strictEqual(packageLock.packages[""].version, "0.10.26");
