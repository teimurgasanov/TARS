...
assert.match(handler, /G\.commitReceiptAcceptanceV1\(e, t,[\s\S]*requireExistingSource: "rejected"/);
assert.doesNotMatch(handler, /G\.writeIndex\(t, G\.PROTECTED_ROOMS\.kassa\.index/);
assert.ok(handler.indexOf("commitReceiptAcceptanceV1") < handler.indexOf("scheduleTarsMemoryHumanReceiptConfirmationV1"), "manual acceptance guard must complete before Memory/report side effects");
